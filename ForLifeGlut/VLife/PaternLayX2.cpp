...
deeptype PaternLayX2::transform1(int x, int y, deeptype h) { // only BASE==2
	...
	if(y >= sz) y -= sz;
	int x2 = x + x, y2 = y + y;
	int ofs1 = Coord2Atom(x2, y2);
	pattertype a = v[ofs1] > 0 ? 1 : 0;
	a += v[ofs1 + 1] > 0 ? 2 : 0;
	int ofs2 = ofs1 + sz;
	a += v[ofs2] > 0 ? 4 : 0;
	a += v[ofs2 + 1] > 0 ? 8 : 0;
	...
	int typeHnew = v[ofs1] = m & 1;
	typeHnew += (v[ofs1 + 1] = m & 2 ? 1 : 0);
	typeHnew += (v[ofs2] = m & 4 ? 1 : 0);
	typeHnew += (v[ofs2 + 1] = m & 8 ? 1 : 0);
...
atype PaternLayX2::RecursGetVal1(int x_v, int y_v) {
	...
	if(dnLay != nullptr) {
		atype sum = dnLay->RecursGetVal1(x_v <<= 1, y_v <<= 1);
		sum += dnLay->RecursGetVal1(x_v + 1, y_v);
		atype nodiag = sum;
		sum += dnLay->RecursGetVal1(x_v, ++y_v);
		sum += dnLay->RecursGetVal1(x_v + 1, y_v);
		if(sum < 2)			return *p0 = 0;
		else if(sum > 2)	return *p0 = 1;
		return *p0 = (nodiag >> 1);	// if(nodiag == 2) :)
	}
	return *p0;
} // ////////////////////////////////////////////////////////////////
atype PaternLayX2::RecursGetVal2(int x_v, int y_v) {
	...
	if(dnLay != nullptr) {
		x_v = (x_v + 1) << 1;
		atype sum = dnLay->RecursGetVal1(x_v-1, y_v <<= 1);	// влево
		if(++x_v >= sz) x_v -= sz;
		sum += dnLay->RecursGetVal1(x_v, y_v);
		atype nodiag = sum;
		sum += dnLay->RecursGetVal1(x_v, ++y_v);
